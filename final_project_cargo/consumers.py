...
from .serializers import Nested_send_cargoSerializer, send_cargoSerializer


class Send_cargo_consumers(AsyncJsonWebsocketConsumer):
    @database_sync_to_async
    def _create_trip(self, data):
        serializer = send_cargoSerializer(data=data)
        serializer.is_valid(raise_exception=True)
        return serializer.create(serializer.validated_data)

    @database_sync_to_async
    def _get_trip_data(self, cargo):
        return Nested_send_cargoSerializer(cargo).data

    @database_sync_to_async
    def _get_trip_ids(self, user):
        user_groups = user.groups.values_list('name', flat=True)
        if 'driver' in user_groups:
            trip_ids = user.Send_cargo_as_driver.exclude(
                status=Send_cargo.COMPLETED
            ).only('id').values_list('id', flat=True)
        else:
            trip_ids = user.Send_cargo_as_rider.exclude(
                status=Send_cargo.COMPLETED
            )
    ...
    def _update_trip(self, data):
        instance = Send_cargo.objects.get(id=data.get('id'))
        serializer = send_cargoSerializer(data=data)
        serializer.is_valid(raise_exception=True)
